...
const DIGEST_NEAREST_OFFSET = 9;
const DIGEST_NEAREST_LENGTH = 5;
const DIGEST_MAX_NEAREST = 3;
...
function processDigest(data) {
  ...
  for(let cNearest = 0; cNearest < DIGEST_MAX_NEAREST; cNearest++) {
    let nearestOffset = DIGEST_NEAREST_OFFSET +
                        (cNearest * DIGEST_NEAREST_LENGTH);
    if(nearestOffset < (frameLength + 2)) {
      let instanceId = data.toString('hex', nearestOffset,
                                     nearestOffset + INSTANCE_LENGTH);
      let interactionCount = toInteractionCount(data.readUInt8(nearestOffset +
                                                              INSTANCE_LENGTH));
      nearest.push({ deviceId: instanceId,
                     interactionCount: interactionCount });
    }
  }

  return { deviceIds: [ instanceId ], nearest: nearest, timestamp: timestamp };
}
