...
const MFR_FRAME_MIN_LENGTH_BYTES = 9;
const MFR_FRAME_NAME_LENGTH_OFFSET = 7;
const MFR_FRAME_NAME_TYPE_OFFSET = 8;
const MFR_FRAME_NAME_OFFSET = 9;
const TEMP_FRAME_MIN_LENGTH_BYTES = 5;
const TEMP_FRAME_NAME_LENGTH_OFFSET = 3;
const TEMP_FRAME_NAME_TYPE_OFFSET = 4;
const TEMP_FRAME_NAME_OFFSET = 5;
const RHT_FRAME_MIN_LENGTH_BYTES = 7;
const RHT_FRAME_TEMP_TYPE_OFFSET = 2;
const RHT_FRAME_TEMP_OFFSET = 3;
const RHT_FRAME_NAME_LENGTH_OFFSET = 5;
const RHT_FRAME_NAME_TYPE_OFFSET = 6;
const RHT_FRAME_NAME_OFFSET = 7;
const MAG_FRAME_MIN_LENGTH_BYTES = 5;
const MAG_FRAME_NAME_LENGTH_OFFSET = 3;
const MAG_FRAME_NAME_TYPE_OFFSET = 4;
const MAG_FRAME_NAME_OFFSET = 5;
const MOV_FRAME_MIN_LENGTH_BYTES = 5;
const MOV_FRAME_NAME_LENGTH_OFFSET = 3;
const MOV_FRAME_NAME_TYPE_OFFSET = 4;
const MOV_FRAME_NAME_OFFSET = 5;
const ANG_FRAME_MIN_LENGTH_BYTES = 9;
const ANG_FRAME_Y_OFFSET = 3;
const ANG_FRAME_Z_OFFSET = 5;
const ANG_FRAME_NAME_LENGTH_OFFSET = 7;
const ANG_FRAME_NAME_TYPE_OFFSET = 8;
const ANG_FRAME_NAME_OFFSET = 9;
const DI_FRAME_MIN_LENGTH_BYTES = 5;
const DI_FRAME_NAME_LENGTH_OFFSET = 3;
const DI_FRAME_NAME_TYPE_OFFSET = 4;
const DI_FRAME_NAME_OFFSET = 5;
const PIR_FRAME_MIN_LENGTH_BYTES = 5;
const PIR_FRAME_NAME_LENGTH_OFFSET = 3;
const PIR_FRAME_NAME_TYPE_OFFSET = 4;
const PIR_FRAME_NAME_OFFSET = 5;
const TOUCH_FRAME_MIN_LENGTH_BYTES = 5;
const TOUCH_FRAME_NAME_LENGTH_OFFSET = 3;
const TOUCH_FRAME_NAME_TYPE_OFFSET = 4;
const TOUCH_FRAME_NAME_OFFSET = 5;
...
function processManufacturerFrame(data) {
  if(data.length < MFR_FRAME_MIN_LENGTH_BYTES) {
    return null;
  }
  ...
  let name = '';
  let isName = (data.readUInt8(MFR_FRAME_NAME_TYPE_OFFSET) === TYPE_NAME_ID);

  if(isName) {
    let nameLength = data.readUInt8(MFR_FRAME_NAME_LENGTH_OFFSET);
    name = data.toString('utf-8', MFR_FRAME_NAME_OFFSET,
                         MFR_FRAME_NAME_OFFSET + nameLength);
  }
...
function processTemperatureFrame(data) {
  if(data.length < TEMP_FRAME_MIN_LENGTH_BYTES) {
    return null;
  }

  let temperature = data.readInt16LE(FRAME_DATA_OFFSET) / 100;
  let name = '';
  let isName = (data.readUInt8(TEMP_FRAME_NAME_TYPE_OFFSET) === TYPE_NAME_ID);

  if(isName) {
    let nameLength = data.readUInt8(TEMP_FRAME_NAME_LENGTH_OFFSET);
    name = data.toString('utf-8', TEMP_FRAME_NAME_OFFSET,
                         TEMP_FRAME_NAME_OFFSET + nameLength);
  }
...
function processTemperatureHumidityFrame(data) {
  if(data.length < RHT_FRAME_MIN_LENGTH_BYTES) {
    return null;
  }
  ...
  if(isTemperature) {
    ...
    let isName = (data.readUInt8(RHT_FRAME_NAME_TYPE_OFFSET) === TYPE_NAME_ID);

    if(isName) {
      let nameLength = data.readUInt8(RHT_FRAME_NAME_LENGTH_OFFSET);
      elaSensor.name = data.toString('utf-8', RHT_FRAME_NAME_OFFSET,
                                     RHT_FRAME_NAME_OFFSET + nameLength);
    }
  }
...
function processMagneticContactFrame(data) {
  if(data.length < MAG_FRAME_MIN_LENGTH_BYTES) {
    return null;
  }
  ...
  let name = '';
  let isName = (data.readUInt8(MAG_FRAME_NAME_TYPE_OFFSET) === TYPE_NAME_ID);

  if(isName) {
    let nameLength = data.readUInt8(MAG_FRAME_NAME_LENGTH_OFFSET);
    name = data.toString('utf-8', MAG_FRAME_NAME_OFFSET,
                         MAG_FRAME_NAME_OFFSET + nameLength);
  }
...
function processMovementDetectionFrame(data) {
  if(data.length < MOV_FRAME_MIN_LENGTH_BYTES) {
    return null;
  }
  ...
  let name = '';
  let isName = (data.readUInt8(MOV_FRAME_NAME_TYPE_OFFSET) === TYPE_NAME_ID);

  if(isName) {
    let nameLength = data.readUInt8(MOV_FRAME_NAME_LENGTH_OFFSET);
    name = data.toString('utf-8', MOV_FRAME_NAME_OFFSET,
                         MOV_FRAME_NAME_OFFSET + nameLength);
  }
...
function processAccelerationFrame(data) {
  if(data.length < ANG_FRAME_MIN_LENGTH_BYTES) {
    return null;
  }
  ...
  let name = '';
  let isName = (data.readUInt8(ANG_FRAME_NAME_TYPE_OFFSET) === TYPE_NAME_ID);

  if(isName) {
    let nameLength = data.readUInt8(ANG_FRAME_NAME_LENGTH_OFFSET);
    name = data.toString('utf-8', ANG_FRAME_NAME_OFFSET,
                         ANG_FRAME_NAME_OFFSET + nameLength);
  }
...
function processDigitalInputFrame(data) {
  if(data.length < DI_FRAME_MIN_LENGTH_BYTES) {
    return null;
  }
  ...
  let name = '';
  let isName = (data.readUInt8(DI_FRAME_NAME_TYPE_OFFSET) === TYPE_NAME_ID);

  if(isName) {
    let nameLength = data.readUInt8(DI_FRAME_NAME_LENGTH_OFFSET);
    name = data.toString('utf-8', DI_FRAME_NAME_OFFSET,
                         DI_FRAME_NAME_OFFSET + nameLength);
  }
...
function processPassiveInfraredFrame(data) {
  if(data.length < PIR_FRAME_MIN_LENGTH_BYTES) {
    return null;
  }
  ...
  let isMotionDetected = ((countAndState % 2) === 1); // Odd = motion
  let name = '';
  let isName = (data.readUInt8(PIR_FRAME_NAME_TYPE_OFFSET) === TYPE_NAME_ID);

  if(isName) {
    let nameLength = data.readUInt8(PIR_FRAME_NAME_LENGTH_OFFSET);
    name = data.toString('utf-8', PIR_FRAME_NAME_OFFSET,
                         PIR_FRAME_NAME_OFFSET + nameLength);
  }
...
function processTouchFrame(data) {
  if(data.length < TOUCH_FRAME_MIN_LENGTH_BYTES) {
    return null;
  }
  ...
  let name = '';
  let isName = (data.readUInt8(TOUCH_FRAME_NAME_TYPE_OFFSET) === TYPE_NAME_ID);

  if(isName) {
    let nameLength = data.readUInt8(TOUCH_FRAME_NAME_LENGTH_OFFSET);
    name = data.toString('utf-8', TOUCH_FRAME_NAME_OFFSET,
                         TOUCH_FRAME_NAME_OFFSET + nameLength);
  }
