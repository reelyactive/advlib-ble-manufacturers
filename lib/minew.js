...
const V3_TEMPERATURE_HUMIDITY_FRAME_LENGTH = 22;
...
function processS3(data) {
  ...
  if((frameVersion === 1) &&      // Static information frame
    ...
    let humidity = utils.parseSigned88(data.subarray(S3_HUMIDITY_OFFSET));
...
function processConnectV3(data) {
  ...
  if((frameVersion === 0) &&      // Static information frame
     (data.length === V3_STATIC_FRAME_LENGTH)) {
  ...
  else if((frameVersion === 5) && // Temperature/humidity frame
     (data.length === V3_TEMPERATURE_HUMIDITY_FRAME_LENGTH)) {
    let temperature = utils.parseSigned88(data.subarray(V3_TEMPERATURE_OFFSET));
    let humidity = utils.parseSigned88(data.subarray(V3_HUMIDITY_OFFSET));
