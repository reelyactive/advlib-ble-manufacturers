...
const V3_00_SALT_OFFSET = 20;
const V3_00_DIGITAL_SIGNATURE_OFFSET = 22;
...
const V3_05_SALT_OFFSET = 20;
const V3_05_DIGITAL_SIGNATURE_OFFSET = 22;
...
const V3_18_SALT_OFFSET = 20;
const V3_18_DIGITAL_SIGNATURE_OFFSET = 22;
const V3_1B_SUBVERSION_OFFSET = 2;
const V3_1B_PRODUCT_ID_OFFSET = 3;
const V3_1B_BLOCKS_OFFSET = 3;
...
const V3_1B_SUBVERSION_MASK = 0x7f;
const V3_1E_DISTANCE_CYCLE_OFFSET = 3;
const V3_1E_DISTANCE_OFFSET = 4;
const V3_1E_FLAGS_OFFSET = 6;
const V3_1E_OCCUPANCY_DETECTION_CYCLE_OFFSET = 7;
const V3_1E_TAMPER_CYCLE_OFFSET = 8;
const V3_1E_SALT_OFFSET = 20;
const V3_1E_DIGITAL_SIGNATURE_OFFSET = 22;
const V3_1E_LOW_BATTERY_MASK = 0x08;
const V3_1E_OCCUPANCY_DETECTION_MASK = 0x04;
const V3_1E_MOTION_DETECTION_MASK = 0x02;
const V3_1E_CONTACT_MASK = 0x01;

const IDENTIFIER_TYPE_EUI_48 = 2;
const MINEW_URI = "https://sniffypedia.org/Organization/Shenzhen_Minew_Technologies_Co_Ltd/";
const MSE_PRODUCT_URIS = new Map([
  [ 0x01, "https://sniffypedia.org/Product/Minew_MSE01/" ],
  [ 0x02, "https://sniffypedia.org/Product/Minew_MSE02/" ]
]);
const V3_PRODUCT_URIS = new Map([
  [ 0x007, "https://sniffypedia.org/Product/Minew_MTB02/" ]
]);
const V3_COMBINATION_BLOCKS = new Map([
  [ 0x03, { length: 7, process: processV3CombinationBlock03 } ],
  [ 0x1f, { length: 5, process: processV3CombinationBlock1F } ],
  [ 0x21, { length: 2, process: processV3CombinationBlock21 } ],
  [ 0x22, { length: 2, process: processV3CombinationBlock22 } ]
]);
const V3_SENSOR_BLOCKS = new Map([
  [ 0x01, { length: 6, process: processV3SensorBlock01 } ],
  [ 0x02, { length: 7, process: processV3SensorBlock02 } ],
  [ 0x03, { length: 3, process: processV3SensorBlock03 } ]
]);
...
function processMSE(data) {
  if(data.length < MSE_MIN_FRAME_LENGTH) {
    return null;
  }

  let model = data.readUInt8(1);
  let txCycle = data.readUInt8(2);
  let batteryPercentage = data.readUInt8(3);
  let sensorStatus = data.readUInt8(4);
  let isButtonPressed = [ (sensorStatus & 0x20) === 0x20,
                          (sensorStatus & 0x40) === 0x40 ];
  let isTamperDetected = [ (sensorStatus & 0x01) === 0x01 ];
  let isMotionDetected = [ (sensorStatus & 0x02) === 0x02 ];
  let energyPercentage = 100 * data.readUInt16LE(5) / 4095;
  let uri = MSE_PRODUCT_URIS.has(model) ? MSE_PRODUCT_URIS.get(model) :
                                          MINEW_URI;
  // TODO: process optional relay data

  return { batteryPercentage: batteryPercentage,
           isButtonPressed: isButtonPressed,
           isTamperDetected: isTamperDetected,
           isMotionDetected: isMotionDetected,
           levelPercentage: energyPercentage,
           txCycle: txCycle, uri: uri };
}
...
function processS3(data) {
  ...
  if((frameVersion === 1) &&      // Static information frame
    ...
    let hardwareVersion = data.readUInt16BE(HARDWARE_VERSION_OFFSET).toString();
...
function processS4(data) {
  ...
  if((frameVersion === 0) &&      // Static information frame
    ...
    return {
        ...
        isTamperDetected: [ !isAntiDisassemblyAlarm ],
        uri: MINEW_URI
    };
  }
...
function processConnectV3(data) {
  ...
  switch(frameVersion) {
    ...
    case 0x1e:
      return processConnectV3Type1e(data);
  }
...
function processConnectV3Type00(data) {
  ...
  let salt = data.readUInt16BE(V3_00_SALT_OFFSET);
  let checksum = data.readUInt16BE(V3_00_DIGITAL_SIGNATURE_OFFSET);

  return {
      batteryPercentage: batteryPercentage,
      deviceIds: [ macSignature ],
      encrypted: { salt: salt, checksum: checksum, method: "minew-connect-v3" },
      firmwareVersion: firmwareVersion,
      uri: MINEW_URI
  };
}
...
function processConnectV3Type03(data) {
  ...
  while(currentBlockOffset < (data.length - V3_03_POST_BLOCK_LENGTH)) {
    ...
    if(V3_COMBINATION_BLOCKS.has(currentBlockId)) {
      let currentBlockParameters = V3_COMBINATION_BLOCKS.get(currentBlockId);
  ...
  if(currentBlockOffset === (data.length - V3_03_POST_BLOCK_LENGTH)) {
    sensorData.encrypted = {
        salt: data.readUInt16BE(currentBlockOffset),
        checksum: data.readUInt16BE(currentBlockOffset + 2),
        method: "minew-connect-v3"
    };
  }
...
function processConnectV3Type05(data) {
  ...
  let salt = data.readUInt16BE(V3_05_SALT_OFFSET);
  let checksum = data.readUInt16BE(V3_05_DIGITAL_SIGNATURE_OFFSET);
  let sensorData = {
      encrypted: { salt: salt, checksum: checksum, method: "minew-connect-v3" },
      name: name,
      temperature: temperature,
...
function processConnectV3Type18(data) {
  ...
  if(subframeVersion === 0x00) {      // Human traffic monitoring
    ...
    sensorData.encrypted = {
        salt: data.readUInt16BE(V3_18_SALT_OFFSET),
        checksum: data.readUInt16BE(V3_18_DIGITAL_SIGNATURE_OFFSET),
        method: "minew-connect-v3"
    };
  }
...
function processConnectV3Type1b(data) {
  if(data.length < V3_1B_SUBVERSION_OFFSET) {
    return null;
  }

  let isEncrypted = (data.readUInt8(V3_1B_SUBVERSION_OFFSET) &
                     V3_1B_ENCRYPTION_MASK) === V3_1B_ENCRYPTION_MASK;
  let subVersion = data.readUInt8(V3_1B_SUBVERSION_OFFSET) &
                   V3_1B_SUBVERSION_MASK;
  ...
  if(subVersion === 0x01) {
    ...
    let temperature = utils.parseSigned88(data.subarray(
                                                    V3_1B_TEMPERATURE_OFFSET));
  ...
  else if(subVersion === 0x02) {
    return processV3SensorBlocks(data.subarray(V3_1B_BLOCKS_OFFSET));
  }
...
/**
 * Process Minew Connect V3 type 0x1e ToF monitor frame data.
 * @param {Object} data The raw manufacturer data as a Buffer.
 * @return {Object} The processed beacon data as JSON.
 */
function processConnectV3Type1e(data) {
  if(data.length !== V3_1E_FRAME_LENGTH) {
    return null;
  }

  let distanceCycle = data.readUInt8(V3_1E_DISTANCE_CYCLE_OFFSET);
  let distance = data.readUInt16LE(V3_1E_DISTANCE_OFFSET) / 1000;
  let flags = data.readUInt8(V3_1E_FLAGS_OFFSET);
  let isBatteryLow = ((flags & V3_1E_LOW_BATTERY_MASK) !== 0);
  let isOccupancyDetected = [ (flags & V3_1E_OCCUPANCY_DETECTION_MASK) !== 0 ];
  let isMotionDetected = [ (flags & V3_1E_MOTION_DETECTION_MASK) !== 0 ];
  let isTamperDetected = [ (flags & V3_1E_CONTACT_MASK) !== 0 ];
  let isOccupancyDetectedCycle = data.readUInt8(
                                       V3_1E_OCCUPANCY_DETECTION_CYCLE_OFFSET);
  let isTamperDetectedCycle = data.readUInt8(V3_1E_TAMPER_CYCLE_OFFSET);
  let salt = data.readUInt16BE(V3_1E_SALT_OFFSET);
  let checksum = data.readUInt16BE(V3_1E_DIGITAL_SIGNATURE_OFFSET);
  ...
  let sensorData = {
      ...
      isTamperDetected: isTamperDetected,
      isMotionDetected: isMotionDetected,
      isOccupancyDetected: isOccupancyDetected,
      isOccupancyDetectedCycle: isOccupancyDetectedCycle,
      isTamperDetectedCycle: isTamperDetectedCycle,
      uri: MINEW_URI
  };
...
/**
 * Process Minew Connect V3 combination block type 0x03 acceleration data.
 * @param {Object} data The raw manufacturer data as a Buffer.
 * @return {Object} The processed beacon data as JSON.
 */
function processV3CombinationBlock03(data) {
  return {
    acceleration: [
...
/**
 * Process Minew Connect V3 combination block type 0x1f door contact data.
 * @param {Object} data The raw manufacturer data as a Buffer.
 * @return {Object} The processed beacon data as JSON.
 */
function processV3CombinationBlock1F(data) {
  let doorSensorStatus = data.readUInt8();

  return { isContactDetected: [ (doorSensorStatus & 0x80) === 0x00 ],
           isTamperDetected: [ (doorSensorStatus & 0x40) === 0x40 ],
           isContactDetectedCycle: data.readUInt8(1),
           isTamperDetectedCycle: data.readUInt8(3) };
}


/**
 * Process Minew Connect V3 combination block type 0x21 battery data.
 * @param {Object} data The raw manufacturer data as a Buffer.
 * @return {Object} The processed beacon data as JSON.
 */
function processV3CombinationBlock21(data) {
  return { batteryPercentage: data.readUInt8() };
}


/**
 * Process Minew Connect V3 combination block type 0x22 tamper data.
 * @param {Object} data The raw manufacturer data as a Buffer.
 * @return {Object} The processed beacon data as JSON.
 */
function processV3CombinationBlock22(data) {
...
/**
 * Process Minew Connect V3 blocks data.
 * @param {Object} data The raw V3 blocks data as a Buffer.
 * @return {Object} The processed beacon data as JSON.
 */
function processV3SensorBlocks(data) {
  let sensorData = { uri: MINEW_URI };
  let currentBlockOffset = 0;

  while(currentBlockOffset < data.length) {
    let currentBlockId = data.readUInt8(currentBlockOffset);

    if(V3_SENSOR_BLOCKS.has(currentBlockId)) {
      let currentBlockParameters = V3_SENSOR_BLOCKS.get(currentBlockId);
      let currentBlockLength = currentBlockParameters.length;
      let currentBlock = data.subarray(currentBlockOffset + 1,
                                       currentBlockOffset + currentBlockLength);
      let blockData = currentBlockParameters.process(currentBlock);
...
/**
 * Process Minew Connect V3 sensor block type 0x01 temperature & humidity data.
 * @param {Object} data The raw manufacturer data as a Buffer.
 * @return {Object} The processed beacon data as JSON.
 */
function processV3SensorBlock01(data) {
  let dataFormat = data.readUInt8();

  if(dataFormat === 0x01) {
    let temperature = (175 * data.readUInt16BE(1) / 65535) - 45;
    let relativeHumidity = (125 * data.readUInt16BE(3) / 65535) - 6;
    return { temperature: temperature, relativeHumidity: relativeHumidity };
  }
...
/**
 * Process Minew Connect V3 sensor block type 0x02 MAC data.
 * @param {Object} data The raw manufacturer data as a Buffer.
 * @return {Object} The processed beacon data as JSON.
 */
function processV3SensorBlock02(data) {
  return { deviceIds: [ toMacSignature(data, 0) ] };
}


/**
 * Process Minew Connect V3 sensor block type 0x03 battery data.
 * @param {Object} data The raw manufacturer data as a Buffer.
 * @return {Object} The processed beacon data as JSON.
 */
function processV3SensorBlock03(data) {
  return { batteryVoltage: data.readUInt16BE() / 1000 };
}
