...
function processRawV2(data) {
  ...
  return {
      ...
      txCycle: measurementSequenceNumber,
      deviceIds: [ deviceId ],
...
function processAir(data) {
  ...
  let volatileOrganicCompoundsIndex = data.readUInt8(AIR_VOC_OFFSET) * 2
                                                       + ((flags & 0x40) >> 6);
  let nitrogenOxidesIndex = data.readUInt8(AIR_NOX_OFFSET) * 2
                                                       + ((flags & 0x80) >> 7);
  let measurementSequenceNumber = data.readUInt8(AIR_SEQ_OFFSET);
  let ruuviAir = { txCycle: measurementSequenceNumber,
  ...
  if(nitrogenOxidesIndex !== 0x1ff) {
    ruuviAir.nitrogenOxidesIndex = nitrogenOxidesIndex;
  }
