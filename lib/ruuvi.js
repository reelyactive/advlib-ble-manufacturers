...
const RUUVI_URI =
              "https://sniffypedia.org/Organization/Ruuvi_Innovations_Ltd_Oy/";
...
function processAir(data) {
  ...
  let temperature = data.readInt16BE(AIR_TEMPERATURE_OFFSET) / 200;
  let relativeHumidity = data.readUInt16BE(AIR_RELATIVE_HUMIDITY_OFFSET) / 400;
  let pressure = data.readUInt16BE(AIR_PRESSURE_OFFSET) + 50000;
  let pm25 = data.readInt16BE(AIR_PM2_5_OFFSET) / 10;
  let carbonDioxideConcentration = data.readInt16BE(AIR_CO2_OFFSET);
  let volatileOrganicCompoundsConcentration = data.readUInt8(AIR_VOC_OFFSET) * 2
                                                       + ((flags & 0x40) >> 6);
  let nitrogenOxideConcentration = data.readUInt8(AIR_NOX_OFFSET) * 2
                                                       + ((flags & 0x80) >> 7);
  let illuminance = Math.exp(data.readUInt8(AIR_LUX_OFFSET) * LUX_DELTA)
                                                               .toFixed(2) - 1;
  ...
  return {
      ...
      volatileOrganicCompoundsConcentration:
                                         volatileOrganicCompoundsConcentration,
      nitrogenOxideConcentration: nitrogenOxideConcentration,
