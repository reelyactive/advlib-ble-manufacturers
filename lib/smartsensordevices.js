...
function processHibouAir(data) {
  ...
  let vocType = data.readUInt8();

  hibouAir.boardId = data.readUIntBE(BOARD_ID_OFFSET, BOARD_ID_LENGTH_BYTES);
  if(boardType === 0x09) {
    hibouAir.soundPressure = data.readUInt16BE(5);
  }
  else {
    hibouAir.illuminance = data.readUInt16LE(5);
  }
  hibouAir.pressure = data.readUInt16LE(7) * 10000;
